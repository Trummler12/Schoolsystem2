...
CSV_HEADERS = {
    ...
    "videos_local.csv": "video_id,language_code,title,description",
    "videos_transcripts.csv": "video_id,language_code,is_generated,is_translatable,transcript",
    "playlists.csv": "playlist_id,channel_id,channel_title,title,description,published_at,item_count,default_language",
...
def ensure_csvs(data_dir: Path, reset: bool) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Batch test YouTube Data API responses.")
    ...
    parser.add_argument("--video-page-limit", type=int, default=2)
    parser.add_argument("--playlist-page-limit", type=int, default=1)
    ...
    existing_channels = read_csv_rows(data_dir / "channels.csv")
    ...
    existing_videos = read_csv_rows(data_dir / "videos.csv")
    ...
    existing_playlists = read_csv_rows(data_dir / "playlists.csv")
    existing_playlists_by_id = {row.get("playlist_id", ""): row for row in existing_playlists if row.get("playlist_id")}

    existing_playlist_items = read_csv_rows(data_dir / "playlistItems.csv")
    ...
    for row in channel_source_rows[start_index:]:
        ...
        for vid in collected_ids:
            if vid and vid not in existing_videos_by_id:
        ...
        write_csv_rows(data_dir / "videos.csv", CSV_HEADERS["videos.csv"].split(","), merged_videos)

        # Playlists
        playlists_resp = api_get(
            "playlists",
        ...
        playlists_items = playlists_resp.get("items", [])
        if args.playlist_page_limit and len(playlists_items) > args.playlist_page_limit * 50:
            playlists_items = playlists_items[: args.playlist_page_limit * 50]
        ...
        write_csv_rows(data_dir / "playlists.csv", CSV_HEADERS["playlists.csv"].split(","), merged_playlists)
        ...
        if args.include_comments and args.comment_video_limit > 0:
            ...
            write_csv_rows(data_dir / "comments.csv", CSV_HEADERS["comments.csv"].split(","), comment_rows)
    ...
    write_csv_rows(data_dir / "channels.csv", CSV_HEADERS["channels.csv"].split(","), merged_channels)

    # Ensure video categories are present in test output
    source_categories = youtube_csv_dir / "videoCategories.csv"
    if source_categories.exists():
        (data_dir / "videoCategories.csv").write_text(
            source_categories.read_text(encoding="utf-8"),
            encoding="utf-8",
        )

    print("OK: test CSVs written to", data_dir)
    return 0
