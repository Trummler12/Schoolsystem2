"""
Generate `ct_resource_tags_PLANNING.csv.txt` by asking Gemini to map resources to tags.

Workflow:
- Load resources from t_source.csv where sa_resource == 1.
- Load tag catalog from t_tag.csv (tagID, name, synonyms).
- For each resource, ask Gemini for 4-7 relevant tagIDs (prefer niche tags).
- Write up to the first 5 tags as rows: resourceID,tagID,weight (weights 5..1).

Supports dry-run (no API calls), resume (skip already written resourceIDs),
and limiting processed rows to conserve quota.
"""
...
DEFAULT_MODEL_A = "gemma-3-12b"
DEFAULT_MODEL_B = "gemma-3-4b"
DEFAULT_MODEL_C = "gemma-3-2b"
DEFAULT_OUTPUT = "../csv/ct_resource_tags_PLANNING.csv.txt"
MIN_TAGS = 4
MAX_TAGS = 7
MAX_WEIGHTS = 5  # we only emit the first 5 tags (weights 5..1)
SAMPLE_SIZE_A = 1  # @CODEX AUFTRAG: Bitte diese 3 Variablen dort einbauen, wo die Anzahl Anfragen pro Modell definiert sind
SAMPLE_SIZE_B = 2
SAMPLE_SIZE_C = 3
WEIGHT_A = 10
WEIGHT_B = 3
...
def resolve_config(args: argparse.Namespace, script_dir: Path) -> Dict[str, str]:
    ...
    # - resources/.env (sibling of scripts/)
    # - main/.env (one level above resources/)
    # - backend/.env (project root for backend)
    # - scripts/.env (same folder as this script)
    search_paths = [
        script_dir / ".env",
    ...
    config: Dict[str, str] = {}
    config["api_key"] = args.api_key or env.get("GEMINI_API_KEY", "")
    config["model"] = args.model or env.get("GEMINI_MODEL", DEFAULT_MODEL_A)
    return config
...
def to_prompt(resource: Resource, tags: Dict[int, Tag]) -> str:
    return build_prompt(resource, tags, desc_limit=2000, include_synonyms=True)
...
def parse_tag_ids(raw_text: str) -> List[int]:
    # Extract JSON array from response (handles code fences or extra text)
...
def call_gemini(
    api_key: str, model: str, prompt: str, api_version: str = "v1", debug: bool = False, _retry_once: bool = True
) -> List[int]:
    ...
    body = {
        ...
        "generationConfig": {
            ...
            "topK": 40,
            "topP": 0.95,
            "maxOutputTokens": 2048, # @CODEX HINWEIS: Token-Auslastung ist momentan bei 13.72K / 250K, also haben wir noch massig Luft nach oben (selbst ein Maximum von 10-20k dürfte kein Problem sein)
        },
    }
    ...
    try:
        ...
        if exc.code == 404 and "not found" in detail.lower():
            if _retry_once and "gemma" in model and api_version == "v1":
                # Auto-fallback to v1beta for Gemma models if not found on v1
                return call_gemini(api_key, model, prompt, api_version="v1beta", debug=debug, _retry_once=False)
            hint = " (model not found; check the exact model name from ListModels)"
        if exc.code == 429 or (exc.code == 403 and "quota" in detail.lower()):
            retry_after = None
            try:
                parsed_detail = json.loads(detail)
                details = parsed_detail.get("error", {}).get("details", [])
                for entry in details:
                    if "retryDelay" in entry:
                        delay = entry.get("retryDelay")
                        if isinstance(delay, str) and delay.endswith("s"):
                            retry_after = float(delay.rstrip("s"))
                        elif isinstance(delay, (int, float)):
                            retry_after = float(delay)
            except Exception:
                pass
            raise RateLimitError(f"Gemini rate limit: {detail}", retry_after=retry_after) from exc
        raise RuntimeError(f"Gemini HTTP error {exc.code}: {detail}{hint}") from exc
    ...
    max_token_reason = False
    for cand in candidates:
        parts = cand.get("content", {}).get("parts") or []
        for part in parts:
            text = part.get("text")
            if isinstance(text, str):
                return parse_tag_ids(text)
        if cand.get("finishReason") == "MAX_TOKENS":
            max_token_reason = True
    # Provide more context when no usable text is returned
    ...
    msg = "No valid text candidates in Gemini response"
    if prompt_feedback:
        msg += f" (promptFeedback={prompt_feedback})"
    elif first_cand:
        msg += (
            f" (first candidate={first_cand}, usage={usage})"
        )
    else:
        msg += f" (raw keys: {list(parsed.keys())})"
    if max_token_reason:
...
def heuristic_tags(resource: Resource, tags: Dict[int, Tag]) -> List[int]:
    # Simple deterministic fallback: choose up to MAX_TAGS smallest IDs whose names appear in text.
...
def fetch_model_lists(
    ...
    for _ in range(repeats):
        while True:
            ...
            try:
                raw = call_gemini(api_key, model, prompt, api_version=api_version, debug=debug)
                ...
                # Propagate so outer loop can count rate-limit hits consistently
                raise
    return results
...
def merge_ranked(weighted_lists: List[Tuple[int, Sequence[int]]]) -> List[int]:
    """
    Combine multiple ranked lists using weighted rank:
    rank = sum((pos in list or len(list)) * weight)
    """
    pos_maps = []
    lengths = []
    weights = []
    ...
    for idx, resource in enumerate(resources, start=1):
        ...
        while True:
            ...
            try:
                ...
                # Do not count this attempt; wait for next window or provided hint
                attempt -= 1
                rate_limit_hits += 1
                if rate_limit_hits > max_rate_limit_retries:
                    raise RuntimeError(
                        f"Resource {resource.source_id} hit rate limits {rate_limit_hits} times; "
                        f"last error: {exc}"
                    ) from exc
                ...
                print(
                    f"[resource {resource.source_id}] MAX_TOKENS, shrinking prompt (desc_limit={desc_limit}, "
                    f"synonyms disabled); retrying after {retry_delay}s",
                    file=sys.stderr,
                )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Assign tags to resources via Gemini and write ct_resource_tags_PLANNING.csv.txt"
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not call Gemini; use heuristic tag selection for quick tests.",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    parser.add_argument(
        "--api-key",
        default=None,
        help="Gemini API key (overrides GEMINI_API_KEY env/.env).",
    )
    parser.add_argument(
        "--model",
        default=None,
        help=f"Gemini model name (default: {DEFAULT_MODEL_A} or GEMINI_MODEL env).",
    )
    parser.add_argument(
        "--api-version",
        default="v1",
        help="Gemini API version to use (default: v1). Gemma models may require v1beta; auto-fallback is attempted.",
    )
    parser.add_argument(
        "--max-attempts",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Maximum Gemini requests per minute for model A; 0 disables rate limiting (default: 30).",
    )
    parser.add_argument(
        "--repeats-a",
        type=int,
...
def main() -> int:
    ...
    if not args.dry_run and not config["api_key"]:
        print("GEMINI_API_KEY is required for live runs. Set env or .env in backend/.", file=sys.stderr)
        return 1
    ...
    model_b = None if not args.secondary_model else args.secondary_model
    model_c = None if not args.tertiary_model else args.tertiary_model
    rate_limiter_b = None if (args.dry_run or not model_b) else RateLimiter(args.secondary_requests_per_minute)
    rate_limiter_c = None if (args.dry_run or not model_c) else RateLimiter(args.tertiary_requests_per_minute)
